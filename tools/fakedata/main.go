...
func envInt(k string, def int) int { if v := os.Getenv(k); v != "" { if x, err := strconv.Atoi(v); err == nil { return x } } ; return def }
func envInt64(k string, def int64) int64 { if v := os.Getenv(k); v != "" { if x, err := strconv.ParseInt(v, 10, 64); err == nil { return x } } ; return def }
...
func envDuration(k string, def time.Duration) time.Duration {
    if v := os.Getenv(k); v != "" { if d, err := time.ParseDuration(v); err == nil { return d } }
    return def
}
    ...
    for i := 0; i < n; i++ {
        ...
        _ = client.UpdateServer(name, func(m *metadata.Metadata) {
            ...
            if rand.Intn(10) == 0 {
                ...
                if cur == "online" { m.SetAnnotation("status", "offline") } else { m.SetAnnotation("status", "online") }
            }
...
func randomPlayerUpdates(client *metadata.Client, serverNames []string) {
    ...
    for uuid := range players { uuids = append(uuids, uuid) }
